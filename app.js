...
document
  .querySelector("#initials-box")
  .addEventListener("click", (event) =>
    toggleMenu(event, document.querySelector("#store-dropdown-container"))
  );
document.querySelector("#initials-box").addEventListener("keyup", (event) => {
  if (event.key === "Enter") {
    toggleMenu(event, document.querySelector("#store-dropdown-container"));
  }
});
...
document.querySelector("#initials").addEventListener("keyup", (event) => {
  if (event.key === "Enter") {
    toggleMenu(event, document.querySelector("#store-dropdown-container"));
  }
});
...
// Close alerts if clicked outside the container
...
function updateProgress() {
  ...
  for (let i = 1; i <= Object.keys(checkboxStates).length; i++) {
    if (checkboxStates[i]) {
      progress += 100 / Object.keys(checkboxStates).length;
    }
  }
...
function toggleTick(checkbox_num) {
  let guideCheckboxTickClass = `.${guides[checkbox_num]}-tick`;
  let checkboxTicks = document.querySelectorAll(guideCheckboxTickClass);

  for (let i = 0; i < checkboxTicks.length; i++) {
    if (checkboxStates[checkbox_num]) {
      checkboxTicks[i].style.display = "block";
    } else {
      checkboxTicks[i].style.display = "none";
    }
  }
}

function toggleCircle(checkbox_num) {
  let guideCheckboxCircleClass = `.${guides[checkbox_num]}-dashed-circle`;
  let checkboxCircles = document.querySelectorAll(guideCheckboxCircleClass);

  for (let i = 0; i < checkboxCircles.length; i++) {
    if (checkboxStates[checkbox_num]) {
      checkboxCircles[i].style.display = "none";
    } else {
      checkboxCircles[i].style.display = "block";
    }
  }
}
...
function checkboxClicked(checkbox_num) {
  ...
  for (let i = 0; checkboxes && i < checkboxes.length; i++) {
    if (checkboxStates[checkbox_num])
      checkboxes[i].style.backgroundColor = "black";
    else checkboxes[i].style.backgroundColor = "transparent";
  }

  if (newCheckboxState) openNextIncomplete();
  updateCompletedGuide();
  updateProgress();
  toggleTick(checkbox_num);
  toggleCircle(checkbox_num);
}
...
for (let i = 1; i <= Object.keys(guides).length; i++) {
  ...
  for (let n = 0; checkboxes && n < checkboxes.length; n++) {
    checkboxes[n].addEventListener("click", (event) => {
      event.stopPropagation();
      let checkbox_num = event.target.parentNode.getAttribute("checkbox_num");
